...
describe('Timeline page testing', () => {
    ...
    it('should remove all', () => {
        ...
        timeline.getAllDisabledValues().should('have.length', 3)
        cy.contains('Remove all').click()
        timeline.getAllDisabledValues().should('have.length', 0)
    })

    it('prevent submiting from empty values', () => {
        timeline.visit()
        timeline.getDataInput().type('2024-07-28')
        timeline.getDataInput().should('have.value', '2024-07-28')

        timeline.addDataToChart('1')

        timeline.getFirstInputForEnteringData().clear()
        timeline.addButtonClick()

        timeline.getFirstInputForEnteringData().should('have.css', 'box-shadow', 'rgb(234, 57, 67) 0px 0px 20px 0px')
    })
    ...
    it('add values to chart by inputing data in inputs, then delete it', () => {
        ...
        timeline.getAllDisabledValues().first().find('[data-cy="inputs-group-close"]').find('span').click()
